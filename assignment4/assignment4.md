...
## Figma
[피그마 링크](https://www.figma.com/design/KIqzrz8a5TtwcrhbDkLRHH/%F0%9F%A7%87-iOS-Seminar-Assignment?node-id=3713-347)

## 기본 스펙
### 전체
- Text, Color, Constraint, Corner Radius 등은 피그마에 주어진 값을 사용합니다
- 상세 디자인 스펙은 피그마에 텍스트로 작성되어 있습니다
- **SwiftUI + MVVM 구조로 구현합니다**
...
### 1. 인기 탭
- 영화 목록 조회에 사용할 [API](https://developer.themoviedb.org/reference/movie-popular-list)
- 평점은 소수점 셋째 자리에서 반올림합니다 (ex. 7.782 -> 7.78)
- 포스터 이미지는 `세로 = 가로 × 1.4`의 비율을 유지한 상태로 화면을 꽉 채웁니다
  - 포스터 이미지가 없는 경우는 고려하지 않습니다

#### 1.1. 영화 상세 화면
- 영화 상세정보 조회에 사용할 [API](https://developer.themoviedb.org/reference/movie-details)
...
### 2. 검색 탭
- 제목 검색에서 사용하는 [API](https://developer.themoviedb.org/reference/search-movie)
- 장르 검색에서 사용하는 [API](https://developer.themoviedb.org/reference/discover-movie)
- 접두어 `장르:`가 붙는 경우에는 장르 검색을 수행하고, 그 외의 경우에 대해서는 제목 검색을 수행합니다 (ex. 검색어 `장르:로맨스`은 장르 검색, 검색어 `로맨스`는 제목 검색)
- View의 상태에 따라 초기화면, 검색 결과(non-Empty), 검색 결과(Empty) 각각을 알맞게 보여줍니다
- Cell의 디자인 스펙은 **인기 탭**에서와 동일합니다
  - 단, Cell을 터치하더라도 상세 페이지로 이동하지 않습니다

### 3. 예매 내역 탭
- 기본 스펙은 예매 내역 탭을 생성하고, 올바른 제목을 설정하는 것까지만 포함합니다 (피그마 내 `MyPage_Empty` 프레임 참고)
